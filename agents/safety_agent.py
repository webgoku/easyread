...
class SafetyAgent(BaseAgent):
    def run(self, original_text: str, simplified_text: str, actions: ActionResult) -> SafetyCheck:
        ...
        dates_orig = _extract_dates(original_text)
        dates_out = _extract_dates(output_combined)
        amounts_orig = _extract_amounts(original_text)
        amounts_out = _extract_amounts(output_combined)
